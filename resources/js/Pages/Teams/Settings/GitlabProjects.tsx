...
export default function GitlabProjects({ team, gitlabProjects, connections, activeConnections }: Props) {
    ...
    const handleConnSubmit = () => {
        if (editingConnection) {
            connForm.put(route('teams.gitlab-connections.update', [team.id, editingConnection.id]), {
    ...
    const handleDeleteConnection = (id: string) => {
        router.delete(route('teams.gitlab-connections.destroy', [team.id, id]), {
    ...
    const handleTestConnection = async (connection: GitlabConnection) => {
        ...
        try {
            const response = await fetch(
                route('teams.gitlab-connections.test', [team.id, connection.id]),
                {
                    method: 'POST',
                    headers: {
        ...
        >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <CardContent>
                        ...
                        {connections.length === 0 ? (
                            <Typography variant="body2" color="text.secondary" sx={{ py: 2 }}>
                                No GitLab connections yet. Add a connection to start linking projects.
                            </Typography>
                        ) : (
                                    ...
                                    <ListItem
                                        ...
                                        secondaryAction={
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                ...
                                                <Tooltip title="Edit">
                                                    <IconButton size="small" onClick={() => openEditConnection(connection)}>
                                                        <EditIcon fontSize="small" />
                                                    </IconButton>
                                                </Tooltip>
                                                <Tooltip title="Delete">
                                                    <IconButton size="small" color="error" onClick={() => setDeleteConnId(connection.id)}>
                                            ...
                                            primary={
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                    ...
                                                    {testResults[connection.id] && (
                                                        <Chip
                                                            icon={testResults[connection.id].success ? <CheckCircleIcon /> : <ErrorIcon />}
                ...
                <Card variant="outlined">
                    <CardContent>
                        ...
                        {activeConnections.length === 0 && connections.length > 0 && (
                            <Alert severity="info" sx={{ mb: 2 }}>
                                No active connections. Enable a connection above to link projects.
                            </Alert>
                        )}
            ...
            <Dialog open={connDialogOpen} onClose={() => setConnDialogOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                        <TextField
                            ...
                            onChange={(e) => connForm.setData('name', e.target.value)}
                        ...
                        <TextField
                            ...
                            onChange={(e) => connForm.setData('base_url', e.target.value)}
                            error={!!connForm.errors.base_url}
                            helperText={connForm.errors.base_url || 'e.g. https://gitlab.example.com'}
                            fullWidth
                            required
                        />
                        <TextField
                            ...
                            onChange={(e) => connForm.setData('api_token', e.target.value)}
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Switch
                                ...
                                onChange={(e) => connForm.setData('is_active', e.target.checked)}
                            />
                ...
                <DialogActions sx={{ px: 3, py: 2 }}>
                    <Button onClick={() => setConnDialogOpen(false)}>Cancel</Button>
                    <Button
                        variant="contained"
            ...
            <Dialog open={!!deleteConnId} onClose={() => setDeleteConnId(null)}>
                ...
                <DialogActions sx={{ px: 3, py: 2 }}>
                    <Button onClick={() => setDeleteConnId(null)}>Cancel</Button>
                    <Button
                        variant="contained"
                        color="error"
                        onClick={() => deleteConnId && handleDeleteConnection(deleteConnId)}
                    >
                        Delete
                    </Button>
                </DialogActions>
            ...
            <Dialog open={!!deleteProjectId} onClose={() => setDeleteProjectId(null)}>
                ...
                <DialogActions sx={{ px: 3, py: 2 }}>
                    ...
                    <Button
                        variant="contained"
                        color="error"
                        onClick={() => deleteProjectId && handleUnlink(deleteProjectId)}
                    >
                        Unlink
                    </Button>
                </DialogActions>

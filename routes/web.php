...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('team.member')->group(function () {
        ...
        // Boards
        Route::post('/teams/{team}/boards', [
            BoardController::class,
            'store',
        ])->name('teams.boards.store');
        Route::get('/teams/{team}/boards/{board}', [
            BoardController::class,
            'show',
        ])->name('teams.boards.show');
        Route::get('/teams/{team}/boards/{board}/tasks', [
            BoardTaskController::class,
            'index',
        ])->name('boards.tasks.index');
        Route::put('/teams/{team}/boards/{board}', [
            BoardController::class,
            'update',
        ])->name('teams.boards.update');
        Route::post('/teams/{team}/boards/{board}/archive', [
            BoardController::class,
            'archive',
        ...
        Route::put('/teams/{team}/boards/{board}/columns/{column}', [
            ColumnController::class,
            'update',
        ])->name('teams.boards.columns.update');
        Route::put('/teams/{team}/boards/{board}/columns', [
            ColumnController::class,
            'reorder',
        ])->name('teams.boards.columns.reorder');
        Route::delete('/teams/{team}/boards/{board}/columns/{column}', [
            ColumnController::class,
            'destroy',
        ])->name('teams.boards.columns.destroy');

        // Tasks
        Route::post('/teams/{team}/boards/{board}/columns/{column}/tasks', [
            TaskController::class,
            'store',
        ])->name('tasks.store');
        Route::get('/teams/{team}/boards/{board}/tasks/{task}', [
            TaskController::class,
            'show',
        ])->name('tasks.show');
        Route::put('/teams/{team}/boards/{board}/tasks/{task}', [
            TaskController::class,
            'update',
        ])->name('tasks.update');
        Route::delete('/teams/{team}/boards/{board}/tasks/{task}', [
            TaskController::class,
            'destroy',
        ])->name('tasks.destroy');
        Route::patch('/teams/{team}/boards/{board}/tasks/{task}/move', [
            TaskController::class,
            'move',
        ])->name('tasks.move');
        Route::put('/teams/{team}/boards/{board}/tasks/{task}/assignees', [
            TaskController::class,
            'updateAssignees',
        ])->name('tasks.assignees.update');
        Route::put('/teams/{team}/boards/{board}/tasks/{task}/labels', [
            TaskController::class,
            'updateLabels',
        ])->name('tasks.labels.update');
        Route::patch(
            '/teams/{team}/boards/{board}/tasks/{task}/toggle-complete',
            [TaskController::class, 'toggleComplete'],
        )->name('tasks.toggle-complete');
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/images', [
            TaskController::class,
            'uploadImage',
        ])->name('tasks.images.store');

        // Task Dependencies
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/dependencies', [
            TaskDependencyController::class,
            'store',
        ])->name('tasks.dependencies.store');
        Route::delete(
            '/teams/{team}/boards/{board}/tasks/{task}/dependencies/{dependsOnTask}',
            [TaskDependencyController::class, 'destroy'],
        )->name('tasks.dependencies.destroy');

        // Task Templates
        Route::get('/teams/{team}/task-templates', [
            TaskTemplateController::class,
            'index',
        ])->name('teams.task-templates.index');
        Route::post('/teams/{team}/task-templates', [
            TaskTemplateController::class,
            'store',
        ])->name('teams.task-templates.store');
        Route::delete('/teams/{team}/task-templates/{taskTemplate}', [
            TaskTemplateController::class,
            'destroy',
        ])->name('teams.task-templates.destroy');
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/save-template', [
            TaskTemplateController::class,
            'createFromTask',
        ])->name('tasks.save-template');
        Route::post(
            '/teams/{team}/boards/{board}/columns/{column}/from-template/{taskTemplate}',
        ...
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/gitlab/branch', [
            TaskGitlabController::class,
            'createBranch',
        ])->name('tasks.gitlab.branch');
        Route::post(
            '/teams/{team}/boards/{board}/tasks/{task}/gitlab/merge-request',
        ...
        // Comments
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/comments', [
            CommentController::class,
            'store',
        ])->name('comments.store');
        Route::put(
            '/teams/{team}/boards/{board}/tasks/{task}/comments/{comment}',
        ...
        // Attachments
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/attachments', [
        ...
        // Team Dashboard
        Route::get('/teams/{team}/dashboard/stats', [
            DashboardController::class,
            'teamStats',
        ])->name('teams.dashboard.stats');
        Route::get('/teams/{team}/export/csv', [
            DashboardController::class,
            'exportCsv',
        ])->name('teams.export.csv');

        // Team Bots & API Tokens
        Route::get('/teams/{team}/bots', [
            TeamBotController::class,
            'index',
        ])->name('teams.bots.index');
        Route::post('/teams/{team}/bots', [
            TeamBotController::class,
            'storeBot',
        ])->name('teams.bots.store');
        Route::delete('/teams/{team}/bots/{user}', [
            TeamBotController::class,
            'destroyBot',
        ])->name('teams.bots.destroy');
        Route::post('/teams/{team}/bots/{user}/tokens', [
            TeamBotController::class,
            'createToken',
        ])->name('teams.bots.create-token');
        Route::delete('/teams/{team}/bots/{user}/tokens/{tokenId}', [
            TeamBotController::class,
            'revokeToken',
        ])->name('teams.bots.revoke-token');

        // Figma Connections (team-scoped)
        Route::get('/teams/{team}/figma', [
            FigmaConnectionController::class,
            'index',
        ])->name('teams.figma.index');
        Route::post('/teams/{team}/figma/connections', [
            FigmaConnectionController::class,
            'store',
        ])->name('teams.figma-connections.store');
        Route::put('/teams/{team}/figma/connections/{figmaConnection}', [
            FigmaConnectionController::class,
            'update',
        ])->name('teams.figma-connections.update');
        Route::delete('/teams/{team}/figma/connections/{figmaConnection}', [
            FigmaConnectionController::class,
            'destroy',
        ])->name('teams.figma-connections.destroy');
        Route::post('/teams/{team}/figma/connections/{figmaConnection}/test', [
            FigmaConnectionController::class,
            'test',
        ])->name('teams.figma-connections.test');

        // Task Figma Links
        Route::get('/teams/{team}/boards/{board}/tasks/{task}/figma', [
            TaskFigmaController::class,
            'index',
        ])->name('tasks.figma.index');
        Route::post('/teams/{team}/boards/{board}/tasks/{task}/figma', [
            TaskFigmaController::class,
            'store',
        ])->name('tasks.figma.store');
        Route::delete(
            '/teams/{team}/boards/{board}/tasks/{task}/figma/{link}',
            [TaskFigmaController::class, 'destroy'],
        )->name('tasks.figma.destroy');

        // GitLab Connections (team-scoped)
        Route::post('/teams/{team}/gitlab/connections', [
            GitlabConnectionController::class,
            'store',
        ])->name('teams.gitlab-connections.store');
        Route::put('/teams/{team}/gitlab/connections/{gitlabConnection}', [
            GitlabConnectionController::class,
            'update',
        ])->name('teams.gitlab-connections.update');
        Route::delete('/teams/{team}/gitlab/connections/{gitlabConnection}', [
            GitlabConnectionController::class,
            'destroy',
        ])->name('teams.gitlab-connections.destroy');
        Route::post(
            '/teams/{team}/gitlab/connections/{gitlabConnection}/test',
            [GitlabConnectionController::class, 'test'],
        )->name('teams.gitlab-connections.test');

        // GitLab Projects
        Route::get('/teams/{team}/gitlab/projects', [
            GitlabProjectController::class,
            'index',
        ])->name('teams.gitlab-projects.index');
        Route::get('/teams/{team}/gitlab/search', [
            GitlabProjectController::class,
            'search',
        ])->name('teams.gitlab-projects.search');
        Route::post('/teams/{team}/gitlab/projects', [
            GitlabProjectController::class,
            'store',
        ])->name('teams.gitlab-projects.store');
        Route::delete('/teams/{team}/gitlab/projects/{gitlabProject}', [
            GitlabProjectController::class,
            'destroy',
        ])->name('teams.gitlab-projects.destroy');

        // Automation Rules
        Route::get('/teams/{team}/boards/{board}/automation-rules', [
            AutomationRuleController::class,
            'index',
        ])->name('boards.automation-rules.index');
        Route::post('/teams/{team}/boards/{board}/automation-rules', [
            AutomationRuleController::class,
            'store',
        ])->name('boards.automation-rules.store');
        Route::put(
            '/teams/{team}/boards/{board}/automation-rules/{automationRule}',
        ...
        // Board Templates
        Route::post('/teams/{team}/boards/{board}/create-template', [
            BoardTemplateController::class,
            'createFromBoard',
        ])->name('boards.create-template');
        Route::post('/teams/{team}/templates/{boardTemplate}/create-board', [
            BoardTemplateController::class,
            'createBoardFromTemplate',
        ])->name('teams.templates.create-board');

        // Saved Filters
        Route::get('/teams/{team}/boards/{board}/filters', [
            SavedFilterController::class,
            'index',
        ])->name('boards.filters.index');
        Route::post('/teams/{team}/boards/{board}/filters', [
            SavedFilterController::class,
            'store',
        ])->name('boards.filters.store');
        Route::put('/teams/{team}/boards/{board}/filters/{savedFilter}', [
            SavedFilterController::class,
            'update',
        ])->name('boards.filters.update');
        Route::delete('/teams/{team}/boards/{board}/filters/{savedFilter}', [
            SavedFilterController::class,
            'destroy',
        ])->name('boards.filters.destroy');

        // Labels
        Route::get('/teams/{team}/labels', [
            LabelController::class,
            'index',
        ])->name('labels.index');
        Route::post('/teams/{team}/labels', [
            LabelController::class,
            'store',
        ])->name('labels.store');
        Route::put('/teams/{team}/labels/{label}', [
            LabelController::class,
            'update',
        ])->name('labels.update');
        Route::delete('/teams/{team}/labels/{label}', [
            LabelController::class,
            'destroy',
        ])->name('labels.destroy');
    });
